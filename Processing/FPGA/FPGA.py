import os
import Processing.FPGA.ModelsimPython as MSP
...
class FPGA():
    def __init__(self,modelsim_path, folder_images_input, folder_images_output) -> None:
        ...
        self.folder_hex = '/'.join(folder_images_input.split('/')[:-1]) + '/FPGA_hex'
        self.folder_out_hex = '/'.join(folder_images_input.split('/')[:-1]) + '/FPGA_out_hex'
        self.folder_img_sim = folder_images_output
        #'/'.join(folder_images_input.split('/')[:-1]) + '/img_processed'

        log.info(f'folder_hex: {self.folder_hex}')
        log.info(f'folder_out_hex: {self.folder_out_hex}')
        log.info(f'folder_img_processed: {self.folder_img_sim}')
    ...
    def generate_tcl(self):
        ...
        with open('./Processing/FPGA/run_cli.tcl','r') as f:
            template_tcl = f.read()
        values = {
            'args' : self.args_path,
    ...
    def file_MPS(self,input_image):
        ...
        img_sim_filename     = os.path.join(self.folder_img_sim , img_src_filename.split('.')[0] + '.png')
        ...
        img_sim = Image.fromarray(img_np_sim).convert('L')

        img_sim.save(img_sim_filename)

        return(img_sim_filename)
    ...
    def __call__(self,input):
        if os.path.isdir(input):
            ...
            return self.file_MPS(input)


if __name__ == '__main__':
    ms = FPGA("/home/ropouillard/intelFPGA/20.1/modelsim_ase/bin/", "./output_elbow_corridor/Camera_01/images")

    ms("./output_elbow_corridor/Camera_01/images/1.png")
